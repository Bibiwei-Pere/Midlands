...
import { useSignupRefferal } from "@/hooks/auth";
import { signIn } from "next-auth/react";

const SignupRefferal = ({ params }: { params: { username: string } }) => {
  ...
  const { mutation, response } = useSignupRefferal(decodeURIComponent(username));

  const form = useForm<z.infer<typeof signupSchema>>({
    resolver: zodResolver(signupSchema),
  });

  const onSubmit = (values: z.infer<typeof signupSchema>) => mutation.mutate(values);
