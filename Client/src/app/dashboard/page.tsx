...
import { CourseCard, CourseCardStats } from "../components/dashboard/CourseCard";
import RecentActivity from "../components/dashboard/RecentActivity";
import Certificates from "../components/dashboard/Certificates";
import { useGetUser } from "@/hooks/users";
import { AlertDialog, AlertDialogContent, AlertDialogTrigger } from "@/components/ui/alert-dialog";
import { RequestPayout } from "../components/dashboard/RequestPayout";
    ...
    return (
      <ContainerDashboard>
        ...
        <div className='flex flex-col gap-6'>
                ...
                <CarouselContent className='flex relative gap-5 overflow-scroll'>
                  {user.activeCourseList.length ? (
                    user.activeCourseList.map((item: any) => (
                      ...
                      >
                        <CourseCard course={item} hide={false} />
                      </CarouselItem>
                    ))
                  ) : (
                    <CourseCard
          ...
          {user.role === "Teacher" && <CourseCardStats />}
